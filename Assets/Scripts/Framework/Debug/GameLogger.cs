 ...
 * 
 *             
 *      Description: 封装Debug日志打印，实现日志开关控制、彩色日志打印、日志文件存储等功能
 *           
 *              
 ***/
using System.IO;
...
public class GameLogger : MonoBehaviour
{
...
#else
        string logDir = string.Format("{0}/gamelog/", Application.persistentDataPath);
#endif
        if (!Directory.Exists(logDir))
        {
            Directory.CreateDirectory(logDir);
        }
        s_logFileSavePath = string.Format("{0}/output_{1}.txt", logDir, time);
    ...
    public static void OnLogCallback(string condition, string stackTrace, LogType type)
    {
        ...
        {
            FileStream fs = File.Create(s_logFileSavePath);
            fs.Close();
        }

        using (StreamWriter sw = File.AppendText(s_logFileSavePath))
        {
